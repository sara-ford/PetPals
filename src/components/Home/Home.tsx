              ...
              {selectedPet.reviews && selectedPet.reviews.length > 0 ? (
                selectedPet.reviews.map((review: any) => (
                  <div key={review.id} className="review">
                    <p className="review-username">נכתב על ידי: {review.username || 'משתמש אנונימי'}</p>
                    ...
                    {editReviewId === review.id ? (
                      <>
                        ...
                        <div className="review-actions">
                          <button onClick={() => saveEditedReview(review)} className="action-button save">
                          ...
                          <button onClick={() => setEditReviewId(null)} className="action-button cancel">
                            <svg viewBox="0 0 24 24" width="16" height="16">
                    ...
                    {String(currentUser?.id) === String(review.userId) && (
                      <div className="review-actions">
                        {editReviewId !== review.id && (
                          <>
                            <button onClick={() => startEditReview(review)} className="action-button edit">
                              <svg viewBox="0 0 24 24" width="16" height="16">
                                <path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>
                              </svg>
                              ערוך
                            </button>
                            <button onClick={() => handleDeleteReview(review.id)} className="action-button delete">
                              <svg viewBox="0 0 24 24" width="16" height="16">
                                <path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/>
                              </svg>
                              מחק
